...
<x-layouts.app :title="__('Dashboard')">
    ...
    <div class="w-full min-h-full">
        ...
        @else
            <div class="flex flex-col items-center justify-center h-96 bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700">
                <flux:icon name="mail-open" class="w-16 h-16 text-zinc-400 dark:text-zinc-600" />
                <flux:heading size="lg" class="mt-4">{{ __('Verify your email address') }}</flux:heading>
                ...
                <form method="POST" action="{{ route('verification.send') }}" class="mt-4">
                    @csrf
                    <flux:button type="submit" variant="primary">{{ __('Resend Verification Email') }}</flux:button>
                </form>
